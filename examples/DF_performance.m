% Three 12m dipoles in Matlab at 12 MHz along cartiesian axes
...
antZ = dipoleCylindrical;
antZ.Length = len;
antZ.Radius = rad;
antX = dipoleCylindrical;
antX.Length = len;
antX.Radius = rad;
antX.Tilt = 90;
antX.TiltAxis = [0, 1, 0];
antY = dipoleCylindrical;
antY.Length = len;
antY.Radius = rad;
antY.Tilt = 270;
antY.TiltAxis = [1, 0, 0];
...
STDs = zeros([2, length(SNRs)]);
    ...
    for t = 1:ntrials
        ...
        ZX = impedance(antX, f);
        ZY = impedance(antY, f);
        ZZ = impedance(antZ, f); %Assumes no matching network, does not matter for now
        VX = ZX * IX;
        VY = ZY * IY;
        VZ = ZZ * IZ;
        V = [VX; VY; VZ];
    ...
    SNR = SNRs(s);
        ...
        theta_err = (thetaDir - thetaDOA) .^2;
        phi_err = (phiDir - phiDOA) .^2;
        STDs(1, s) = STDs(1, s) + theta_err;
        STDs(2, s) = STDs(2, s) + phi_err;
    end


end

STDs = sqrt(STDs ./ ntrials);
%%
close

plot(10 * log10(SNRs), 2*STDs, '-x', 'LineWidth', 2)
title(sprintf('Direction-finding accuracy estimates \n(1000 realizations each)'))
legend({'Theta', 'Phi'})
xlabel('SNR (dB)')
ylabel('95% Confidence (deg)')
grid on
grid minor
