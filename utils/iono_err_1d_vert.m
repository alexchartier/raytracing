function cost = iono_err_1d_vert(X, rays, nhops, tol, alts, lats, lons, Bx, By, Bz)
%% define cost function for retrieval
% X is a vector of length 3X1 used to define the ionospheric params
...
X3 = repmat(X, 1, length(lats) * length(lons));
[iono_en_grid, iono_grid_parms] = gen_ionosphere_from_coeffs(X3(:), alts, lats, lons);
...
NmF2 = max(iono_en_grid(1, 1, :));
hmF2 = alts(iono_en_grid(1, 1, :) == NmF2);
fprintf('NmF2: %1.1e, hmF2: %1.0f\n', NmF2, hmF2)
