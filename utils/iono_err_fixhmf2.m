function cost = iono_err_fixhmf2(X_in, hmf2, rays, nhops, tol, alts, lats, lons, Bx, By, Bz)
%% define cost function for retrieval
% X is a vector of length 3X npts used to define the ionospheric params
...
%% Expand out X with the hmF2
npts = length(lats) * length(lons);
X = [X_in(1:npts); ones(npts, 1) * hmf2 / 300; X_in(npts+1:end)];
