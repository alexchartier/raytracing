...
%
%    $Id: madDownloadFile.m 7438 2022-05-20 13:55:34Z brideout $
%
result = -1;
...
% try to use wget
[result, cmdout] = system('/opt/homebrew/bin/wget -h');
if (result ~= 0)
    exception = MException('Madmatlab:WgetFailed', ...
       'Unable to run /opt/homebrew/bin/wget - please make sure its installed');
    throw(exception);
end
...
wget_command = horzcat('/opt/homebrew/bin/wget -q --timeout=600 --tries=4 -O', ' ', outputFile, ' "', cgiurl, '"');
...
if (fileType == -1)
    ...
    if (result ~= 0)
        fprintf(1, "gunzip is not installed, file %s may still be compressed", outputFile);
    else
